...
uint16_t ViewUVTimeEvt(uint8_t add, uint16_t key)
{
  ...
  if (treeView[add].keyMode != menuIdxLast)
    menuValueSet = 0;
  if (sys.uvTime / HOUR > 9999)
    sys.uvTime = sys.uvTime * HOUR;
  u8g2_SetFont(&u8g2, u8g2_font_unifont_t_symbols);
  u8g2_DrawGlyph(&u8g2, 55, 54, 0x23f0);
  sprintf((char *)buff, "%04d Hours", sys.uvTime / HOUR);
  u8g2_SetFont(&u8g2, u8g2_font_6x12_mf);
  u8g2_DrawStr(&u8g2, ((22 - strlen("Lamp Time Worked!")) / 2) * 6, 24, "Lamp Time Worked!");
...
uint16_t ViewFilterTimeEvt(uint8_t add, uint16_t key)
{
  ...
  if (treeView[add].keyMode != menuIdxLast)
    menuValueSet = 0;
  if (sys.filterTime / HOUR > 9999)
    sys.filterTime = sys.filterTime * HOUR;
  u8g2_SetFont(&u8g2, u8g2_font_unifont_t_symbols);
  u8g2_DrawGlyph(&u8g2, 55, 54, 0x23f0);
  sprintf((char *)buff, "%04d Hours", sys.filterTime / HOUR);
  u8g2_SetFont(&u8g2, u8g2_font_6x12_mf);
  u8g2_DrawStr(&u8g2, ((22 - strlen("AirFan Time Worked!")) / 2) * 6, 24, "AirFan Time Worked!");
  u8g2_DrawStr(&u8g2, ((22 - strlen(buff)) / 2) * 6, 36, buff);
...
uint16_t ViewSystemTimeEvt(uint8_t add, uint16_t key)
{
  ...
  if (treeView[add].keyMode != menuIdxLast)
    menuValueSet = 0;
  if (sys.sysTime / HOUR > 99999)
    sys.sysTime = sys.sysTime * HOUR;
  u8g2_SetFont(&u8g2, u8g2_font_unifont_t_symbols);
  u8g2_DrawGlyph(&u8g2, 55, 54, 0x23f0);
    ...
    else
    {
      ...
      if (menuValueSet == sys_cfg.passWord)
      {
        if (key == BTN_MENU)
        {
          ...
          u8g2_DrawStr(&u8g2, ((22 - strlen(buff)) / 2) * 6, 36, buff);
          sprintf((char *)buff, "\4 Reset Lamp Time!\n");
          u8g2_DrawStr(&u8g2, ((22 - strlen("Reset Lamp Time!")) / 2) * 6, 48, "Reset Lamp Time!");
        }
      }
      else
      {
    ...
    else
    {
      ...
      if (menuValueSet == sys_cfg.passWord)
      {
        if (key == BTN_MENU)
        {
          ...
          //sprintf((char*)buff, "%04d Hours", sys.UVTime);
          u8g2_DrawStr(&u8g2, ((22 - strlen(buff)) / 2) * 6, 36, buff);
          sprintf((char *)buff, "\4 Reset AirFan Time!\n");
          u8g2_DrawStr(&u8g2, ((22 - strlen("Reset AirFan Time!")) / 2) * 6, 48, "Reset AirFan Time!");
        }
      }
      else
      {
