...
static float toFlow(float pressure);
...
void SM5852_Manager(SM5852_T *sm5852)
{
	...
	if(HAL_GetTick() - sm5852->tick > 100)
	{
		...
		sm5852->sum = sm5852->sum + (psi * 1034.21)/P_MAX;
		sm5852->state++;
		//sm5852->pressure = (psi * 1034.21)/P_MAX; 					// Convert psi to Pa
		//printf("Pressure: %.2f\n", sm5852->pressure);
		if(sm5852->state == SS_SAMPLE)
		{
			sm5852->pressure = sm5852->sum/SS_SAMPLE;
			sm5852->sum = 0;
			sm5852->state = 0;
		}
		else
		{
			sm5852->pressure = sm5852->pressure;
		}
		temp = toTemp(sm5852->buff[3], sm5852->buff[2]);
		temp = Update_Estimate(temp);
		temp = Update_Estimate(temp);
		sm5852->temp = Update_Estimate(temp);
		sm5852->flow = toFlow(sm5852->pressure);
		//printf("Temp: %.2f\n", sm5852->temp);
		sm5852->tick = HAL_GetTick();
	}
}
...
static float toPressure(uint8_t hi, uint8_t lo)
{
  ...
  /* *------*	*------* 	*------* */
  float pressure = (((float)pressureCnt - DigoutPmin)/((DigoutPmax - DigoutPmin)/0.19));
  return pressure;
}
...
static float toTemp(uint8_t hi, uint8_t lo)
{
  ...
  //float temp = (((float)tempCnt - DigoutTmin)/((DigoutTmax - DigoutTmin)/(150 + 30)) - 30);
  float temp = ((DigoutTmax - (float)tempCnt)/((DigoutTmax - DigoutTmin)/(150 + 30)) - 90);
  return temp;
}

static float toFlow(float pressure)
{
	return (pressure * 0.8 / 480);
}