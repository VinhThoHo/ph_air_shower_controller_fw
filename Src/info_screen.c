...
static void DateTime_Display(void)
{
    ...
    u8g2_DrawStr(&u8g2, (((25 - strlen(buff)) / 2) * 4) + 1, 14, buff);
    u8g2_SetDrawColor(&u8g2, 1);
    u8g2_SetFont(&u8g2, u8g2_font_5x8_tf);
    sprintf(buff, "%02d/%02d/%04d\n", localTime.mday, localTime.month, localTime.year);
    u8g2_DrawStr(&u8g2, (((22 - strlen(buff)) / 2) * 8), 27, buff);
}
...
static void UV_Time(uint32_t time)
{
    ...
    u8g2_DrawStr(&u8g2, 2 + 15, 40, "LAMP      :");
    // u8g2_SetDrawColor(&u8g2, 0);
    sprintf(buff, "%04d", time);
    u8g2_DrawStr(&u8g2, 25 + 50, 40, buff);
    // u8g2_SetDrawColor(&u8g2, 1);
    u8g2_DrawStr(&u8g2, 55 + 50, 40, "h");
}
...
static void Filter_Time(uint32_t time)
{
    ...
    u8g2_DrawStr(&u8g2, 2 + 15, 50, "AIR NOZZLE:");
    // u8g2_SetDrawColor(&u8g2, 0);
    sprintf(buff, "%04d", time);
    u8g2_DrawStr(&u8g2, 25 + 50, 50, buff);
    // u8g2_SetDrawColor(&u8g2, 1);
    u8g2_DrawStr(&u8g2, 55 + 50, 50, "h");
}
...
static void Sys_Time(uint32_t time)
{
    uint8_t buff[5];
    u8g2_SetFont(&u8g2, u8g2_font_5x7_mf);
    u8g2_DrawStr(&u8g2, 2 + 15, 60, "SYSTEM    :");
    // u8g2_SetDrawColor(&u8g2, 0);
    sprintf(buff, "%05d", time);
    u8g2_DrawStr(&u8g2, 25 + 50, 60, buff);
    // u8g2_SetDrawColor(&u8g2, 1);
    u8g2_DrawStr(&u8g2, 55 + 50, 60, "h");
}
